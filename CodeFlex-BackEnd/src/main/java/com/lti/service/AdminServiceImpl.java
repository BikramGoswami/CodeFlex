...
public class AdminServiceImpl implements AdminService{
	...
	@Autowired
	private AdminLoginRepository userPassRepo;
	...
	public int updatePassword(String email, String password) {
			AdminLoginDetail user = userPassRepo.findByEmail(email);
			if(user==null)
				throw new UserServiceException("The email id you entered is not registered with us!");
			user.setPassword(password);
			AdminLoginDetail updatedUser = em.merge(user);
			return updatedUser.getAdminId();
	}
